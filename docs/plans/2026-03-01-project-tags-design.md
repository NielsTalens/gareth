...
## Goal
Make it easy to test gentlii with multiple product definitions by selecting a test project in the UI. Each project corresponds to a folder in `projects/` containing only `.md` product definition files. The selected project is sent alongside the feature proposal to the OpenAI API.
